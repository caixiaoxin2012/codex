    ...
    def run(self) -> None:
        try:
            ...
            if self.path.is_dir() and any(self.path.rglob("*.xml")):
                ...
                self.progress.emit(25, "正在执行 SCL 大小/SHA-256 安全检查…")
                project = SecureProjectAnalyzer().scan(self.path)
...
class MainWindow(BaseMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("SCL AI Analyzer V0.11.3 — Unified Secure Loader")
    ...
    def start_analysis(self) -> None:
        ...
        self.log("INFO", "开始项目分析（SCL/XML 统一执行大小、SHA-256 与安全解析）")
        self._thread = QThread(self)
        self._worker = UnifiedSecureAnalysisWorker(Path(self.project_path))
    ...
    def on_progress(self, value: int, message: str) -> None:
        ...
        level = "WARNING" if message.startswith("安全加载提示：") else "INFO"
